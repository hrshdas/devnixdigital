...
import { Share2, CheckCircle, ArrowLeft, Instagram, Facebook, Twitter, TrendingUp, Users, BarChart3, Sun, Moon, X, Menu, ExternalLink } from 'lucide-react';
...
const SocialMediaPage = () => {
  ...
  useEffect(() => {
    ...
    document.title = "Social Media Management - DevNix Digital";
  ...
  const features = [
    {
      ...
      description: "Comprehensive content strategies that align with your brand voice and engage your target audience effectively."
    },
    {
      icon: <Facebook className="w-6 h-6" />,
      title: "Content Planning & Scheduling",
      description: "Strategic content calendar planning and automated scheduling across all major social media platforms."
    },
    {
      icon: <Twitter className="w-6 h-6" />,
      title: "Community Management",
      description: "Active community engagement, responding to comments, messages, and building meaningful relationships with your audience."
    },
    {
      icon: <Users className="w-6 h-6" />,
      title: "Visual Content Creation",
      description: "Eye-catching graphics, videos, and visual content that stops the scroll and drives engagement."
    },
    {
      icon: <TrendingUp className="w-6 h-6" />,
      title: "Social Media Advertising",
      description: "Targeted social media ad campaigns that reach your ideal customers and drive conversions."
    },
    {
      icon: <Share2 className="w-6 h-6" />,
      title: "Influencer Marketing",
      description: "Strategic partnerships with influencers and content creators to expand your brand reach and credibility."
    }
  ];

  const benefits = [
    "Increase follower growth by 400%",
    ...
    >
      ...
      <section className={`relative min-h-screen flex items-center justify-center overflow-hidden ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
        ...
        <div 
          ...
          style={{
            backgroundImage: `url('https://images.unsplash.com/photo-1611224923853-80b023f02d71?q=80&w=1539&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')`
          }}
        ></div>
        
        {/* Lighter Gradient Overlay with Blur */}
        <div className={`absolute inset-0 backdrop-blur-sm ${isDarkMode ? 'bg-gradient-to-br from-gray-900/60 via-gray-800/40 to-gray-900/50' : 'bg-gradient-to-br from-blue-500/40 via-indigo-400/30 to-blue-600/35'}`}></div>
        
        {/* Enhanced Soft Color Overlay with Gradient and Blur */}
        <div className={`absolute inset-0 backdrop-blur-[2px] ${isDarkMode ? 'bg-gradient-to-tr from-gray-700/15 via-transparent to-gray-600/10' : 'bg-gradient-to-tr from-blue-200/25 via-blue-100/15 to-indigo-200/20'}`}></div>

        {/* Back to Home Button - Top Left Corner */}
        <div className="absolute top-6 left-6 z-20">
          <Link 
            to="/" 
            className="flex items-center text-white/90 hover:text-white transition-colors group bg-white/10 backdrop-blur-sm px-4 py-2 rounded-full shadow-lg hover:shadow-xl border border-white/20"
          >
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          ...
          >
            <div className="w-20 h-20 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-2xl flex items-center justify-center mx-auto mb-8 shadow-2xl">
              <Users className="w-10 h-10 text-white" />
            </div>
            
            <h1 className="text-5xl md:text-6xl font-bold text-white mb-6">
              Social Media <span className="bg-gradient-to-r from-blue-300 to-indigo-300 bg-clip-text text-transparent">Management</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link to="/#portfolio" className="border-2 border-white/30 text-white px-8 py-4 rounded-xl font-semibold hover:bg-white/10 transition-all duration-300 backdrop-blur-sm">
                See Our Work
              </Link>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <h2 className={`text-4xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} mb-6`}>
                Build Thriving Communities with 
                <span className={`bg-gradient-to-r from-blue-500 to-indigo-600 ${isDarkMode ? 'bg-clip-text text-transparent' : 'bg-clip-text text-transparent'}`}> Strategic Social Media</span>
              </h2>
              <p className={`text-lg ${isDarkMode ? 'text-white/80' : 'text-gray-600'} mb-6 leading-relaxed`}>
                Our social media management goes beyond just posting content. We create comprehensive strategies that build authentic communities, foster meaningful engagement, and drive real business results across all major social platforms.
              </p>
              <p className={`text-lg ${isDarkMode ? 'text-white/80' : 'text-gray-600'} mb-8 leading-relaxed`}>
                From content creation and community management to influencer partnerships and paid social campaigns, we handle every aspect of your social media presence so you can focus on running your business.
              </p>
              
              <div className="grid grid-cols-2 gap-6">
                <div className={`text-center p-4 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-sm`}>
                  <div className={`text-3xl font-bold ${isDarkMode ? 'text-blue-600' : 'text-blue-600'} mb-2`}>400%</div>
                  <div className={`text-sm ${isDarkMode ? 'text-white/80' : 'text-gray-600'}`}>Follower Growth</div>
                </div>
                <div className={`text-center p-4 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-sm`}>
                  <div className={`text-3xl font-bold ${isDarkMode ? 'text-blue-600' : 'text-blue-600'} mb-2`}>250%</div>
                  <div className={`text-sm ${isDarkMode ? 'text-white/80' : 'text-gray-600'}`}>Engagement Boost</div>
                </div>
              </div>
            </motion.div>
                ...
                <ul className="space-y-4">
                  {benefits.map((benefit, index) => (
                    <li key={index} className="flex items-center">
                      <CheckCircle className="w-5 h-5 mr-3 text-blue-200 flex-shrink-0" />
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          >
            <h2 className={`text-4xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} mb-6`}>
              ...
              <span className={`bg-gradient-to-r from-blue-500 to-indigo-600 ${isDarkMode ? 'bg-clip-text text-transparent' : 'bg-clip-text text-transparent'}`}> Services</span>
            </h2>
            <p className={`text-xl ${isDarkMode ? 'text-white/80' : 'text-gray-600'} max-w-3xl mx-auto`}>
              Complete social media solutions that build brand awareness, engage your audience, and drive business growth.
            </p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {features.map((feature, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className={`group p-8 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 hover:border-blue-200`}
              >
                <div className={`w-14 h-14 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center ${isDarkMode ? 'text-white' : 'text-white'} mb-6 group-hover:scale-110 transition-transform duration-300`}>
                  {feature.icon}
                </div>
                <h3 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} mb-4 group-hover:text-blue-600 transition-colors`}>
                  {feature.title}
                </h3>
      ...
      {/* CTA Section */}
      <section className={`py-20 ${isDarkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-700'} relative overflow-hidden`}>
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-indigo-400/30 rounded-full mix-blend-multiply filter blur-xl animate-pulse delay-1000"></div>
        </div>
        ...
        <div className="relative max-w-4xl mx-auto px-6 text-center">
          ...
          >
            ...
            <p className={`text-xl ${isDarkMode ? 'text-blue-100' : 'text-blue-100'} mb-8 max-w-2xl mx-auto`}>
              Let's create a social media strategy that builds authentic connections and drives real business results. Start growing your community today.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link 
                to="/#contact"
                className={`bg-white ${isDarkMode ? 'text-blue-600' : 'text-blue-600'} px-8 py-4 rounded-xl font-semibold hover:bg-blue-50 transition-all duration-300 shadow-lg hover:shadow-xl`}
              >
                Get Started Today
              </Link>
              <Link 
                to="/"
                className={`bg-blue-500/30 backdrop-blur-sm ${isDarkMode ? 'text-white' : 'text-white'} px-8 py-4 rounded-xl font-semibold hover:bg-blue-500/40 transition-all duration-300 border border-white/20`}
              >
                View All Services
              </Link>
            </div>
